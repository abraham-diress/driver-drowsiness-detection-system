...
Step 1: setupCameraAndDetectors Implementation
    Using cv2

Step 2: performDrowsinessDetection
    * Face Extract
    * Take only Face
...
Step 3: Alarm
    # Final : after counting of 5 
    # Trigger Alarm Function
'''


class DrowsinessperformDrowsinessDetectionSystem:
    def __init__(self):
        super(DrowsinessperformDrowsinessDetectionSystem, self).__init__()
        self.faceCascade = "../input/haarcascade_frontalface_default.xml"
        self.eyeCascade = "../input/haarcascade_lefteye_2splits.xml"
        self.loadModel = "../input/drowiness_detection_system_model.h5" 
    ...
    def setupCameraAndDetectors(self):
        # load face_cascade
    ...
    def performDrowsinessDetection(self):
        # load model
        model = tf.keras.models.load_model(self.loadModel)
        cap, face_cascade, left_eye_cascade = self.setupCameraAndDetectors()
        while True:
            _, frame = cap.read()
            ...
            # scaleFactor=1.3 specifies how much the image size is reduced with each scale. In a group photo, there may be some faces which are near the setupCameraAndDetectors than others. Naturally, such faces would appear more prominent than the ones behind. This factor compensates for that
            ...
            for (x, y, w, h) in faces:
                ...
                image_pixels = img_to_array(roi_color) # convert to array
                    ...
                    if maxIndex2 == 2:
                        ...
                        if self.counting > 5:
                            ...
                            if not self.alarm_on:
                                ...
                                t = Thread(target=self.playAlarmSound()) # Thread for multi function run
                                t.daemon = True
                                t.start()
...
if __name__ == '__main__':
    a = DrowsinessperformDrowsinessDetectionSystem()
    a.performDrowsinessDetection()